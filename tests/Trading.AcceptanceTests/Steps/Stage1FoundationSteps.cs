...
public sealed class Stage1FoundationSteps(Stage1ScenarioState state)
{
    ...
    private bool HandleRepository(string text)
    {
        ...
        if (text.StartsWith("no Stage 1 scenario", StringComparison.Ordinal)) { Assert.That(Directory.GetFiles(Path.Combine(Root, "tests", "Trading.AcceptanceTests", "Features"), "*.feature", SearchOption.AllDirectories).All(f => !File.ReadAllText(f).Contains("@ignore", StringComparison.Ordinal))); return true; }
