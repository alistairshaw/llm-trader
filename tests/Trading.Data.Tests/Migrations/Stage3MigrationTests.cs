...
internal sealed class Stage3MigrationTests
{
    ...
    public async Task FreshCreationAndReapplicationProduceTheCompleteStageThreeSchema()
    {
        ...
        {
            Assert.That(await ScalarAsync<long>(database.Context, "SELECT COUNT(*) FROM __ef_migrations_history"), Is.EqualTo(17));
            Assert.That(await ScalarAsync<string>(database.Context, "SELECT value FROM schema_metadata WHERE key='application_data_format_version'"), Is.EqualTo("7"));
    ...
    public async Task CompletedStageTwoFixtureUpgradesWithoutChangingAnyExistingRow()
    {
        ...
        try
        {
            ...
            Assert.That(await ScalarAsync<long>(context, "SELECT COUNT(*) FROM __ef_migrations_history"), Is.EqualTo(17));
            Assert.That(await ScalarAsync<string>(context, "SELECT value FROM schema_metadata WHERE key='application_data_format_version'"), Is.EqualTo("7"));
        }
        finally
        {
