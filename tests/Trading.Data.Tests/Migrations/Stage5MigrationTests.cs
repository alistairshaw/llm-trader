...
internal sealed class Stage5MigrationTests
{
    ...
    public async Task FreshAndCompletedStageFourUpgradeHaveEquivalentSchemaAndRetainHistory()
    {
        ...
        Assert.Multiple(async () => { Assert.That(await SchemaAsync(upgraded.Context), Is.EqualTo(schema)); Assert.That(await ScalarAsync<string>(upgraded.Context, "SELECT id||'|'||content_hash FROM research_reports"), Is.EqualTo(before)); Assert.That(await ScalarAsync<long>(upgraded.Context, "SELECT COUNT(*) FROM research_report_sources"), Is.EqualTo(1)); Assert.That(await ScalarAsync<long>(upgraded.Context, "SELECT COUNT(*) FROM __ef_migrations_history"), Is.EqualTo(13)); Assert.That(await ScalarAsync<string>(upgraded.Context, "SELECT value FROM schema_metadata WHERE key='application_data_format_version'"), Is.EqualTo("6")); });
    }
