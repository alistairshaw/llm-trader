...
The proposal-governance orchestrator acquires state before each evaluation, persists the initial decision, applies an authorized exact-content human decision, and then acquires and evaluates fresh state again before requesting an atomic reservation. An approved proposal retains its approval during a passing revalidation and becomes rejected when that fresh evaluation fails. Retries reconstruct proposal, evaluation, approval, and reservation state from durable identities; expiration processing is idempotent and releases capacity through the reservation service. Provider and model calls remain outside repository transactions, and this Stage 5 graph ends at reservation without an order or broker port.
