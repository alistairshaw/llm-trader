...
The WPF `TradingApplicationLifecycle` captures the host's canonical database identity before startup. Lifecycle stop first awaits hosted-service shutdown and asynchronously disposes the host/root provider, then clears only that host database's exact closed SQLite pool before reporting `Stopped`. Repeated stop and disposal calls do not repeat ownership disposal or affect unrelated database pools.
...
`LLM_TRADER_WPF_EXECUTABLE` path to the published deterministic artifact. Commit CI selects `HarnessSmoke` and runs
the complete Stage 7 WPF category twice; Linux and
