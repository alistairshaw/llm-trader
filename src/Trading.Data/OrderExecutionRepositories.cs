...
public sealed class OrderWorkRepository(TradingDbContext db) : IOrderWorkRepository
{
    ...
    { var row = await db.OutboxMessages.SingleOrDefaultAsync(x => x.Id == id.ToString(), token).ConfigureAwait(false); if (row is null || row.Status != "Claimed" || row.LeaseOwner != owner) return new PersistenceWriteResult.ConcurrencyConflict(0, row?.Version); row.Status = complete ? "Completed" : "Pending"; row.LastError = detail; row.CompletedAt = complete ? UtcUnixMilliseconds.ToProvider(at) : null; row.AvailableAt = complete ? row.AvailableAt : UtcUnixMilliseconds.ToProvider(at); row.LeaseOwner = null; row.LeaseExpiresAt = null; row.Version++; return await RepositoryWrites.SaveAsync(db, "outbox_identity", token).ConfigureAwait(false); }
...
public sealed class BrokerInboxRepository(TradingDbContext db) : IBrokerInboxRepository
{
    ...
    private async Task<PersistenceWriteResult> Update(BrokerMessageId id, string owner, string detail, DateTimeOffset at, bool complete, CancellationToken token) { var row = await db.InboxMessages.SingleOrDefaultAsync(x => x.Id == id.ToString(), token).ConfigureAwait(false); if (row is null || row.Status != "Claimed" || row.LeaseOwner != owner) return new PersistenceWriteResult.ConcurrencyConflict(0, row?.Version); row.Status = complete ? "Completed" : "Pending"; row.LastError = detail; row.CompletedAt = complete ? UtcUnixMilliseconds.ToProvider(at) : null; row.AvailableAt = complete ? row.AvailableAt : UtcUnixMilliseconds.ToProvider(at); row.LeaseOwner = null; row.LeaseExpiresAt = null; row.Version++; return await RepositoryWrites.SaveAsync(db, "inbox_identity", token).ConfigureAwait(false); }
}
