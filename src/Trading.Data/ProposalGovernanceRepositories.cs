...
internal static class ProposalPersistenceMapper
{
    ...
    internal static async Task<TradeProposal?> LoadProposalAsync(TradingDbContext db, string id, CancellationToken token)
    {
        ...
        return TradeProposal.Rehydrate(new(TradeProposalId.Parse(row.Id), TradingBotId.Parse(row.TradingBotId), BotRunId.Parse(row.BotRunId), PortfolioId.Parse(row.PortfolioId), TradingBotConfigurationVersionId.Parse(row.ConfigurationVersionId), PortfolioDecisionSnapshotId.Parse(row.PortfolioSnapshotId), InstrumentId.Parse(row.InstrumentId), requested, row.Rationale, new(action.ContentVersion, action.ContentHash), hypothesis, reportRows.Select(x => new ReportEvidenceReference(ResearchReportId.Parse(x.Id), x.ReportSeriesId, x.VersionNumber, x.ContentHash)).ToArray(), Status(row.Status), UtcUnixMilliseconds.FromProvider(row.CreatedAt), UtcUnixMilliseconds.FromProvider(row.ValidUntil), row.Version, evaluations.Select(ToState).ToArray(), approvals.Select(x => ToState(x, action)).ToArray()));
    }
    internal static GuardrailEvaluationEntity ToEntity(string proposalId, GuardrailEvaluation value) => new() { Id = value.Id.ToString(), TradeProposalId = proposalId, EvaluationSequence = value.Sequence, EvaluationStage = value.EvaluationStage, PolicyVersion = value.PolicyVersion, Outcome = CanonicalEnumeration.Format(value.Outcome), StateSnapshotId = value.StateSnapshotId.ToString(), RuleResultsJson = CanonicalJsonSerializer.Serialize(Schema, new EvaluationDto(value.RuleResults.Select(x => new RuleDto(x.Rule, x.Outcome.ToString(), x.Reason)).ToArray(), value.PolicyReference?.Level.ToString(), value.PolicyReference?.PolicyId, value.FreshState?.ObservedAt, value.FreshState?.ContentHash)), EvaluatedAt = UtcUnixMilliseconds.ToProvider(value.EvaluatedAt) };
    private static GuardrailEvaluationState ToState(GuardrailEvaluationEntity row) { var dto = CanonicalJsonSerializer.Deserialize<EvaluationDto>(Schema, row.RuleResultsJson); var policy = dto.PolicyLevel is null ? null : new GuardrailPolicyReference(CanonicalEnumeration.Parse<GuardrailPolicyLevel>(dto.PolicyLevel), dto.PolicyId!, row.PolicyVersion); var fresh = dto.ObservedAt is null ? null : new FreshStateReference(PortfolioDecisionSnapshotId.Parse(row.StateSnapshotId), dto.ObservedAt.Value, dto.StateHash!); return new(GuardrailEvaluationId.Parse(row.Id), row.EvaluationSequence, row.EvaluationStage, row.PolicyVersion, CanonicalEnumeration.Parse<GuardrailOutcome>(row.Outcome), dto.Rules.Select(x => new GuardrailRuleResult(x.Rule, CanonicalEnumeration.Parse<GuardrailOutcome>(x.Outcome), x.Reason)).ToArray(), UtcUnixMilliseconds.FromProvider(row.EvaluatedAt), PortfolioDecisionSnapshotId.Parse(row.StateSnapshotId), policy, fresh); }
    ...
    private static ProposalApprovalState ToState(ProposalApprovalEntity row, ActionDto action) => new(ProposalApprovalId.Parse(row.Id), CanonicalEnumeration.Parse<ApprovalDecision>(row.Decision), CanonicalEnumeration.Parse<ApprovalActorType>(row.ActorType), row.ActorId, row.Reason, UtcUnixMilliseconds.FromProvider(row.DecidedAt), row.ProposalVersion, PortfolioDecisionSnapshotId.Parse(row.StateSnapshotId), new(action.ContentVersion, action.ContentHash), null);
    ...
    private sealed record RuleDto(string Rule, string Outcome, string Reason);
    private sealed record EvaluationDto(RuleDto[] Rules, string? PolicyLevel, string? PolicyId, DateTimeOffset? ObservedAt, string? StateHash);
}
