...
internal static class ProposalPersistenceMapper
{
    ...
    {
        DirectTradeAction x => CanonicalJsonSerializer.Serialize(Schema, new ActionDto("DirectTrade", x.Side.ToString(), x.Quantity.Amount, x.Quantity.Unit, x.OrderType, x.LimitPrice?.Amount, x.LimitPrice?.Currency.Code, x.TimeInForce, null, value.ContentVersion.Version, value.ContentVersion.ContentHash, value.ExecutionMode.ToString())),
        TargetAllocationAction x => CanonicalJsonSerializer.Serialize(Schema, new ActionDto("TargetAllocation", null, null, null, null, null, null, null, x.TargetPercentage.Value, value.ContentVersion.Version, value.ContentVersion.ContentHash, value.ExecutionMode.ToString())),
    ...
    internal static async Task<TradeProposal?> LoadProposalAsync(TradingDbContext db, string id, CancellationToken token)
    {
        ...
        return TradeProposal.Rehydrate(new(TradeProposalId.Parse(row.Id), TradingBotId.Parse(row.TradingBotId), BotRunId.Parse(row.BotRunId), PortfolioId.Parse(row.PortfolioId), TradingBotConfigurationVersionId.Parse(row.ConfigurationVersionId), PortfolioDecisionSnapshotId.Parse(row.PortfolioSnapshotId), InstrumentId.Parse(row.InstrumentId), requested, row.Rationale, new(action.ContentVersion, action.ContentHash), hypothesis, reportRows.Select(x => new ReportEvidenceReference(ResearchReportId.Parse(x.Id), x.ReportSeriesId, x.VersionNumber, x.ContentHash)).ToArray(), Status(row.Status), UtcUnixMilliseconds.FromProvider(row.CreatedAt), UtcUnixMilliseconds.FromProvider(row.ValidUntil), row.Version, evaluationStates, approvals.Select(x => ToState(x, action, evaluationStates)).ToArray(), action.ExecutionMode is null ? ExecutionMode.HumanApproval : CanonicalEnumeration.Parse<ExecutionMode>(action.ExecutionMode)));
    }
    ...
    private sealed record ActionDto(string Kind, string? Side, decimal? Quantity, string? QuantityUnit, string? OrderType, decimal? LimitPrice, string? LimitCurrency, string? TimeInForce, decimal? TargetPercentage, int ContentVersion, string ContentHash, string? ExecutionMode = null);
